<template>
    <div class="flex justify-between bg-white border-2 rounded-md w-full mb-5 shadow-lg p-3" :class="{'bg-gray-200': !props.spot.active}">
        ...
        <div class="icons flex justify-between w-24 self-center p-2">
            ...
            <i v-if="props.spot.active" class="fas fa-trash cursor-pointer text-white bg-red-500 p-2 rounded-md hover:bg-red-600" @click="deleteSpot()"></i>
        </div>
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const deleteSpot = async () => {
            confirm(`Está seguro de que desea desactivar el espacio ${props.spot?.spotName}?`) ? 
                await db.collection('ParkingSpots').doc(props.spot?.id.toString()).update({active: false})
                : null;
